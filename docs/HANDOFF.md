...
28. **M24（用户"收口版"提示词：权限引导 + 功能正确性 + 安全兜底，目标"能真正给别人用"）**：先派只读审计 agent 摸清"已实现 vs 缺口"（结论：§3 删除安全、Docker `system df`、9/12 残留位置、逐项确认、完成汇总均已实现），只补真实缺口。**§1+§6 全局 FDA 引导（最大缺口，原来只接在清理 tab）**：新增 `Permissions/FullDiskAccessSupport.swift`（`PermissionActions` 深链系统设置/重启 + `FullDiskAccessBanner` 所有 tab 顶部持久横幅）、`Permissions/WelcomeSheet.swift`（首启欢迎：安全承诺 + FDA 请求）；`RootView` 挂横幅 + `didBecomeActive` 复查 + 首启 sheet；`SettingsView` 加权限状态区；`AppSettings.hasCompletedOnboarding`；删掉 CleanView 自带整页 gate 与孤立的 `PermissionOnboardingView.swift`。**§3.4**：清理/构建产物/卸载完成弹窗加「打开废纸篓」入口（`SystemActions.openTrash`，Docker 不加——运行时资源走 CLI 删除不进废纸篓）。**§2.1**：`FileSizeCalculator.sizeAsync/size` 从逻辑 `.fileSize` 改成递归累加 `allocatedSize`（真实可释放空间、稀疏安全），同步改 4 个精确小体积测试对照 `allocatedSize`；`PurgeScanner.defaultRoots` 加 `workspace`。**§4**：`MountainSilhouette` 填充 0.09→0.06；三个清理 hero 未扫描态去掉"--"虚线环（改成扫描中转圈/未扫描只有文字）。**足够/不做并记录**：§2.2 受保护项已"禁用+给原因"（未拆独立分区）；§2.5 缺的 LaunchDaemons/receipts 是 root 属主删不掉、Group Containers 共享数据误删风险大（沿用既有安全决策跳过）；§2.6 sparkline 已是真实 60 秒采样（冷启动点少才像直线，非 bug）。146 测试通过、Release 构建通过、离屏渲染确认、装机冒烟无 crash。
